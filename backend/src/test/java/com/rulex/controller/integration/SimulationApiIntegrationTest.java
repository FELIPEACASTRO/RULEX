...
/**
 * 🔥 TESTES INTEGRADOS - API DE SIMULAÇÃO Testa RuleSimulationController,
 * HomologSimulationController, RulesV31Controller
 */
...
public class SimulationApiIntegrationTest extends BaseIntegrationTest {
  ...
  class TestRulesTests {
    ...
    void shouldSimulateNormalTransaction() throws Exception {
      ...
      performPost("/api/simulation/test", request).andExpect(status().isOk());
    }
    ...
    void shouldSimulateBatchTransactions() throws Exception {
      ...
      performPost("/api/simulation/batch", request).andExpect(status().isOk());
    }
    ...
    void shouldCompareRules() throws Exception {
      ...
      performPost("/api/simulation/compare", request).andExpect(status().isOk());
    }
  }
  ...
  class RulesV31Tests {
    ...
    void shouldValidateValidRule() throws Exception {
      ...
      performPost("/api/rules-v31/validate", rule).andExpect(status().isOk());
    }
    ...
    void shouldLintRule() throws Exception {
      ...
      performPost("/api/rules-v31/lint", rule).andExpect(status().isOk());
    }
    ...
    void shouldSimulateV31Rule() throws Exception {
      ...
      performPost("/api/rules-v31/simulate", request).andExpect(status().isOk());
    }
  }
  ...
  class HomologSimulationTests {
    ...
    void shouldRunHomologSimulation() throws Exception {
      ...
      performPost("/api/homolog/simulation/run", request).andExpect(status().isOk());
    }
  }
