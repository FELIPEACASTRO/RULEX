...
/**
 * 🔥 TESTES INTEGRADOS - API DE AUDITORIA E MÉTRICAS Testa AuditController, MetricsController,
...
public class AuditAndMetricsApiIntegrationTest extends BaseIntegrationTest {
  ...
  class AuditTests {
    ...
    void shouldListAllAuditLogs() throws Exception {
      performGet("/api/audit").andExpect(status().isOk());
    }
    ...
    void shouldFilterByAction() throws Exception {
      mockMvc
          .perform(
              get("/api/audit").param("action", "CREATE").contentType(MediaType.APPLICATION_JSON))
          .andExpect(status().isOk());
    }
    ...
    void shouldExportAuditJson() throws Exception {
      mockMvc
          .perform(get("/api/audit/export").contentType(MediaType.APPLICATION_JSON))
          .andExpect(status().isOk());
    }
    ...
    void shouldExportAuditCsv() throws Exception {
      mockMvc.perform(get("/api/audit/export/csv").accept("text/csv")).andExpect(status().isOk());
    }
    ...
    void shouldGetAuditByTransaction() throws Exception {
      ...
      performGet("/api/audit/transaction/" + transactionId).andExpect(status().isOk());
    }
  }
  ...
  class RuleMetricsTests {
    ...
    void shouldGetDashboard() throws Exception {
      performGet("/api/rule-metrics/dashboard").andExpect(status().isOk());
    }

    @Test
    @DisplayName("✅ Cenário 2: Obter métricas de todas as regras")
    void shouldGetAllRuleMetrics() throws Exception {
      performGet("/api/rule-metrics/all").andExpect(status().isOk());
    }
    ...
    void shouldGetSpecificRuleMetrics() throws Exception {
      ...
      performGet("/api/rule-metrics/" + ruleId).andExpect(status().isOk());
    }
