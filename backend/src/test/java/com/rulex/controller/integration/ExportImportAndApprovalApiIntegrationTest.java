...
/**
 * 🔥 TESTES INTEGRADOS - API DE EXPORT/IMPORT E APROVAÇÃO Testa RuleExportImportController,
...
public class ExportImportAndApprovalApiIntegrationTest extends BaseIntegrationTest {
  ...
  class ExportTests {
    ...
    void shouldExportAllRules() throws Exception {
      performGet("/api/rules/export").andExpect(status().isOk());
    }
    ...
    void shouldExportSelectedRules() throws Exception {
      ...
      request.put("ruleIds", List.of(UUID.randomUUID().toString(), UUID.randomUUID().toString()));

      performPost("/api/rules/export/selective", request).andExpect(status().isOk());
    }

    @Test
    @DisplayName("✅ Cenário 3: Exportar regras complexas")
    void shouldExportComplexRules() throws Exception {
      performGet("/api/rules/export/complex").andExpect(status().isOk());
    }
  }
  ...
  class ImportTests {
    ...
    void shouldImportRulesJson() throws Exception {
      ...
      performPost("/api/rules/import", importData).andExpect(status().isOk());
    }
    ...
    void shouldImportRulesFile() throws Exception {
      ...
      MockMultipartFile file =
          new MockMultipartFile(
              "file", "rules.json", MediaType.APPLICATION_JSON_VALUE, jsonContent.getBytes());

      mockMvc.perform(multipart("/api/rules/import/file").file(file)).andExpect(status().isOk());
    }
    ...
    void shouldRejectInvalidFile() throws Exception {
      MockMultipartFile file =
          new MockMultipartFile("file", "invalid.txt", "text/plain", "invalid content".getBytes());

      mockMvc
          .perform(multipart("/api/rules/import/file").file(file))
  ...
  class ApprovalTests {
    ...
    void shouldCreateApprovalForNewRule() throws Exception {
      ...
      performPost("/api/approval/create", request).andExpect(status().isOk());
    }
    ...
    void shouldCreateApprovalForUpdate() throws Exception {
      ...
      performPost("/api/approval/update/" + ruleId, request).andExpect(status().isOk());
    }
    ...
    void shouldCreateApprovalForDelete() throws Exception {
      ...
      performPost("/api/approval/delete/" + ruleId, request).andExpect(status().isOk());
    }
    ...
    void shouldApproveRequest() throws Exception {
      ...
      performPost("/api/approval/" + approvalId + "/approve", request).andExpect(status().isOk());
    }
    ...
    void shouldRejectRequest() throws Exception {
      ...
      performPost("/api/approval/" + approvalId + "/reject", request).andExpect(status().isOk());
    }
  }
  ...
  class HomologRulesTests {
    ...
    void shouldCreateHomologRule() throws Exception {
      ...
      performPost("/api/homolog/rules", rule).andExpect(status().isOk());
    }
    ...
    void shouldGetLatestRuleVersion() throws Exception {
      String ruleId = UUID.randomUUID().toString();
      performGet("/api/homolog/rules/" + ruleId + "/latest").andExpect(status().isOk());
    }
  ...
  private Map<String, Object> createImportData() {
    ...
    rule1.put(
        "conditions", List.of(Map.of("fieldName", "amount", "operator", "GT", "value", "1000")));
    rules.add(rule1);
  ...
  private Map<String, Object> createNewRuleApprovalRequest() {
    ...
    rule.put(
        "conditions", List.of(Map.of("fieldName", "amount", "operator", "GT", "value", "5000")));
  ...
  private Map<String, Object> createHomologRule() {
    ...
    rule.put(
        "conditions", List.of(Map.of("fieldName", "amount", "operator", "GT", "value", "10000")));
    return rule;
  }
}
