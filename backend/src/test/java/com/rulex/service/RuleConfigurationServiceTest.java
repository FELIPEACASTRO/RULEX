...
class RuleConfigurationServiceTest {
  ...
  @Mock private RuleConfigurationRepository ruleConfigRepository;

  @Mock private RuleConfigurationHistoryRepository historyRepository;
  ...
  void setUp() {
    ...
    ruleConfigurationService =
        new RuleConfigurationService(
            ruleConfigRepository, historyRepository, auditService, objectMapper);
  }
  ...
  void shouldListAllRulesWithPagination() {
    List<RuleConfiguration> rules =
        Arrays.asList(createTestRule("RULE_1"), createTestRule("RULE_2"));
  ...
  void shouldThrowNotFoundExceptionWhenRuleNotFound() {
    ...
    assertThrows(NotFoundException.class, () -> ruleConfigurationService.getRuleById(999L));
  }
  ...
  void shouldThrowExceptionWhenCreatingDuplicateRule() {
    ...
    assertThrows(IllegalStateException.class, () -> ruleConfigurationService.createRule(dto));
  }
  ...
  void shouldToggleRuleEnabledStatus() {
    ...
    when(ruleConfigRepository.findById(1L)).thenReturn(Optional.of(rule));
    when(ruleConfigRepository.save(any(RuleConfiguration.class)))
