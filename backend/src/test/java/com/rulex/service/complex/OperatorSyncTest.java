...
/**
 * TRIPLE CHECK AVASSALADOR - Testes de Sincronização de Operadores
 * Verifica que TODOS os operadores estão sincronizados entre:
 * - Entity (RuleCondition.ConditionOperator)
 * - DTO (ConditionDTO.OperatorType)
...
public class OperatorSyncTest {
    ...
    void testEntityAndDtoOperatorsMatch() {
        ...
        assertTrue(onlyInEntity.isEmpty(), 
            "❌ GAPS ENCONTRADOS! Operadores na Entity mas não no DTO: " + onlyInEntity);
        assertTrue(onlyInDto.isEmpty(), 
            "❌ GAPS ENCONTRADOS! Operadores no DTO mas não na Entity: " + onlyInDto);
        
        assertEquals(entityOps.size(), dtoOps.size(), 
            "Entity e DTO devem ter o mesmo número de operadores");
    ...
    void testAllOperatorsHaveEvaluateMethod() {
        ...
        assertTrue(evaluateMethods.size() >= 400, 
    ...
    void testOperatorCount() {
        ...
        assertTrue(entityCount >= 440, 
            "Entity deve ter pelo menos 440 operadores. Encontrados: " + entityCount);
        assertTrue(dtoCount >= 440, 
            "DTO deve ter pelo menos 440 operadores. Encontrados: " + dtoCount);
    ...
    void testValueTypesSync() {
        ...
        assertEquals(entityValueTypes, dtoValueTypes, 
            "Entity e DTO devem ter o mesmo número de ValueTypes");
