-- ============================================================================
-- Migration V50: Reaplicar regras simples no rule_configurations real
-- Data: 2026-01-29
-- Objetivo: Corrigir V40 (temp table) e persistir as 100 regras simples
-- ============================================================================

CREATE TEMP TABLE rule_configurations_seed (
...
INSERT INTO rule_configurations_seed (rule_name, description, field_name, operator, comparison_value, decision_outcome, score_impact, priority, status, shadow_mode, created_by)
...
-- ============================================================================
-- Insert into real rule_configurations
-- ============================================================================

INSERT INTO public.rule_configurations
  (rule_name, description, enabled, weight, classification, rule_type, threshold, parameters, conditions_json, logic_operator, shadow_mode, canary_percentage, version)
SELECT
  rule_name,
  description,
  (status = 'ACTIVE') AS enabled,
  score_impact AS weight,
  CASE decision_outcome
    WHEN 'APPROVE' THEN 'APPROVED'
    WHEN 'REVIEW' THEN 'SUSPICIOUS'
    WHEN 'BLOCK' THEN 'FRAUD'
  END AS classification,
  'CONTEXT' AS rule_type,
  score_impact AS threshold,
  jsonb_build_object(
    'priority', priority,
    'status', status,
  ...
  'AND' AS logic_operator,
  'DISABLED' AS shadow_mode,
  0 AS canary_percentage,
  1 AS version
FROM rule_configurations_seed
