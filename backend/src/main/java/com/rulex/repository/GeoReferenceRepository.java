...
public interface GeoReferenceRepository extends JpaRepository<GeoReference, Long> {
     ...
     */
    @Query("SELECT g FROM GeoReference g WHERE g.countryCode = :countryCode " +
    ...
    /**
     * Busca a referência mais próxima de um ponto (para fallback).
     * Usa distância euclidiana simplificada.
     */
    @Query(value = "SELECT * FROM geo_reference " +
     ...
     */
    @Query("SELECT g FROM GeoReference g WHERE " +
