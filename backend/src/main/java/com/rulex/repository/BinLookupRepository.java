...
public interface BinLookupRepository extends JpaRepository<BinLookup, Long> {
    ...
    /**
     * Busca BIN por prefixo (para BINs de 6 ou 8 dígitos).
     * Tenta primeiro com 8 dígitos, depois com 6.
     */
    @Query("SELECT b FROM BinLookup b WHERE b.bin = :bin8 OR b.bin = :bin6 ORDER BY LENGTH(b.bin) DESC")
