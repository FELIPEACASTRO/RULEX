...
public interface RuleListEntryRepository extends JpaRepository<RuleListEntry, Long> {
  ...
  @Query(
      "SELECT e FROM RuleListEntry e WHERE e.list.id = :listId AND e.isActive = true "
          + "AND (e.expiresAt IS NULL OR e.expiresAt > :now)")
  List<RuleListEntry> findActiveNonExpiredEntries(
      @Param("listId") Long listId, @Param("now") LocalDateTime now);

  /** Verifica se valor está em alguma blacklist ativa para o tipo de entidade. */
  @Query(
      "SELECT COUNT(e) > 0 FROM RuleListEntry e "
          + "JOIN e.list l "
  ...
  @Modifying
  @Query(
      "UPDATE RuleListEntry e SET e.isActive = false WHERE e.expiresAt < :now AND e.isActive = true")
