...
public interface RuleMetricsRepository extends JpaRepository<RuleMetrics, Long> {
   ...
   */
  @Query("SELECT m FROM RuleMetrics m WHERE m.metricDate BETWEEN :startDate AND :endDate " +
   ...
   */
  @Query("SELECT m.ruleId, m.ruleName, SUM(m.totalEvaluations), SUM(m.totalTriggered), " +
   ...
   */
  @Query("SELECT m FROM RuleMetrics m WHERE m.metricDate = :date " +
   ...
   */
  @Query("SELECT SUM(m.totalAmountBlocked) FROM RuleMetrics m " +
