 ...
 *
 * <ul>
 *   <li>Compare new rule version vs existing (CONTROL vs TREATMENT)
 *   <li>Statistical significance calculation
 *   <li>Sticky assignment - same PAN always gets same variant
 *   <li>Automatic winner detection
 * </ul>
 *
 * <p>Key Metrics Tracked:
 *
 * <ul>
 *   <li>Trigger Rate: % of transactions triggering the rule
 *   <li>False Positive Rate: Triggers on legitimate transactions
 *   <li>True Positive Rate: Catches on confirmed fraud
 *   <li>Latency: Performance impact of rule
 * </ul>
 */
@Service
...
public class ABTestingService {
  ...
  public static class ExperimentMetrics {
    ...
    public Map<String, Object> toMap() {
      ...
      return Map.of(
          ...
          trigs,
          "triggerRate",
          evals > 0 ? (double) trigs / evals * 100 : 0,
          "truePositives",
          truePositives.get(),
          "falsePositives",
          falsePositives.get(),
          "avgScore",
          trigs > 0 ? (double) totalScore.get() / trigs : 0,
          "avgLatencyMicros",
          evals > 0 ? (double) totalLatencyMicros.get() / evals : 0);
    }
  }

  /** Assignment result. */
  public record ExperimentAssignment(
      Long experimentId, ExperimentGroup group, Long ruleIdToEvaluate, LocalDateTime assignedAt) {}
  ...
  public Experiment createExperiment(
    ...
    log.info(
        "Created A/B test experiment: {} (control={}, treatment={})",
        name,
        controlRuleId,
        treatmentRuleId);
  ...
  public Experiment startExperiment(Long experimentId) {
    ...
    Experiment updated =
        new Experiment(
            ...
            experiment.controlRuleId(),
            experiment.treatmentRuleId(),
  ...
  /** Gets experiment assignment for a transaction. Uses sticky assignment based on PAN hash. */
  public Optional<ExperimentAssignment> getAssignment(String panHash, Long ruleId) {
    ...
    Optional<Experiment> experiment =
        activeExperiments.values().stream()
    ...
    Long ruleIdToEvaluate =
        group == ExperimentGroup.CONTROL ? exp.controlRuleId() : exp.treatmentRuleId();

    ExperimentAssignment assignment =
        new ExperimentAssignment(exp.id(), group, ruleIdToEvaluate, LocalDateTime.now());
  ...
  public void recordEvaluation(
      Long experimentId, ExperimentGroup group, boolean triggered, int score, long latencyMicros) {
    Experiment experiment = activeExperiments.get(experimentId);
        ...
        .forEach(
            e -> {
              ...
              if (experiment != null && experiment.status() == ExperimentStatus.RUNNING) {
                ExperimentMetrics metrics =
                    assignment.group() == ExperimentGroup.CONTROL
                        ? experiment.controlMetrics()
                        : experiment.treatmentMetrics();
  ...
  /**
   * Assigns group based on deterministic hash of PAN. This ensures same PAN always gets same group.
   */
  ...
  private String generateRecommendation(
      Experiment experiment, String winner, boolean isSignificant) {
    long totalEvals =
        experiment.controlMetrics().evaluations.get()
