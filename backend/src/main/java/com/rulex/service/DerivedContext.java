 ...
 * <p>Deriva:
 * <ul>
 *   <li>transactionDate (Integer YYYYMMDD) → LocalDate</li>
 *   <li>transactionTime (Integer HHMMSS) → LocalTime</li>
 *   <li>(date + time + gmtOffset) → OffsetDateTime (transaction_ts)</li>
 *   <li>pan → masked_pan, bin (primeiros 6-8 dígitos), last4</li>
 *   <li>merchantPostalCode → normalized (apenas dígitos)</li>
 *   <li>merchantState → normalized (uppercase)</li>
 *   <li>merchantCountryCode → normalized (3 dígitos)</li>
 * </ul>
 * 
 * <p>Todos os métodos têm fallback seguro (retornam Optional.empty() ou valor padrão)
 * para não quebrar o fluxo em caso de erro de parse.
 */
@Data
@Builder
@Slf4j
public class DerivedContext {
    ...
    /**
     * Cria um DerivedContext a partir do payload original.
     * Erros de parse são logados como WARN e não interrompem o fluxo.
     */
    public static DerivedContext from(TransactionRequest request) {
        ...
        OffsetDateTime timestamp = deriveTimestamp(localDate, localTime, request.getGmtOffset()).orElse(null);
    ...
    /**
     * Combina LocalDate + LocalTime + gmtOffset para criar OffsetDateTime.
     * gmtOffset esperado no formato "-0300" ou "+0530".
     * @return Optional.empty() se qualquer componente for inválido
     */
    public static Optional<OffsetDateTime> deriveTimestamp(LocalDate date, LocalTime time, String gmtOffset) {
    ...
    /**
     * Mascara o PAN mantendo apenas primeiros 6 e últimos 4 dígitos.
     * Ex: "4111111111111111" → "411111******1111"
     */
    public static String maskPan(String pan) {
        ...
        return digitsOnly.substring(0, 6) + "*".repeat(Math.max(0, maskLength)) + digitsOnly.substring(digitsOnly.length() - 4);
    }

    /**
     * Extrai o BIN (Bank Identification Number) do PAN.
     * Retorna os primeiros 6-8 dígitos dependendo do comprimento.
     */
    ...
    /**
     * Normaliza estado para uppercase.
     * Ex: "sp" → "SP"
     */
    ...
    /**
     * Normaliza código de país para 3 dígitos.
     * Ex: "76" → "076", "076" → "076"
     */
