 ...
 *
 * <ul>
 *   <li>In-memory HyperLogLog-like counting for real-time velocity
 *   <li>Sliding window algorithm with bucketed counters
 *   <li>Periodic sync to database for persistence
 * </ul>
 *
 * <p>Performance characteristics:
 *
 * <ul>
 *   <li>Read latency: O(1) - sub-microsecond
 *   <li>Write latency: O(1) - sub-microsecond
 *   <li>Memory: ~100 bytes per unique key per window
 * </ul>
 *
 * <p>For production with multiple instances, replace with Redis implementation:
 *
 * <ul>
 *   <li>Redis Sorted Sets for time-windowed counting
 *   <li>Redis HyperLogLog for distinct counts
 *   <li>Redis Streams for event sourcing
 * </ul>
 *
...
public class RedisVelocityService {
  ...
  /**
   * Records a transaction for velocity tracking. Call this AFTER the transaction is processed.
   *
   * @param request Transaction request
   */
  ...
  private void recordToCounters(
    ...
    // Record sum
    sumCounters.computeIfAbsent(cacheKey, k -> new SlidingWindowSum()).add(timestamp, amount);

    // Record distinct values
    if (request.getMerchantId() != null) {
      distinctCounters
          .computeIfAbsent(cacheKey + ":merchants", k -> new HyperLogLogCounter())
          .add(request.getMerchantId());
    }
    if (request.getMcc() != null) {
      distinctCounters
          .computeIfAbsent(cacheKey + ":mccs", k -> new HyperLogLogCounter())
  ...
  /** Sliding window counter using bucketed approach. Each bucket represents 1 minute of data. */
  ...
  /**
   * Simplified HyperLogLog-like counter for distinct value estimation. Uses a probabilistic
   * approach with configurable precision.
   *
   * <p>For production, use Redis HyperLogLog or Google's HyperLogLogPlusPlus.
   */
  private static class HyperLogLogCounter {
    ...
    private static int murmurHash3(String value) {
      ...
      switch (len) {
        case 3:
          k ^= (data[i + 2] & 0xff) << 16; // fall through
        case 2:
          k ^= (data[i + 1] & 0xff) << 8; // fall through
        case 1:
          k ^= (data[i] & 0xff);
          k *= 0xcc9e2d51;
          k = Integer.rotateLeft(k, 15);
