 ...
 *
 * <p>Implementa: - GEO_DISTANCE_LT/GT: Distância Haversine entre dois pontos - GEO_IN_POLYGON:
 * Verificação de ponto dentro de polígono (ray casting)
 *
 * <p>Como o payload não pode ser alterado, as coordenadas são derivadas de: - merchantCity +
 * merchantState + merchantCountryCode → lookup na tabela geo_reference - Fallback para centroid do
 * país se cidade não encontrada
 */
@Service
...
public class GeoService {
  ...
  /**
   * Deriva coordenadas de uma transação. Usa merchantCity, merchantState, merchantCountryCode para
   * lookup.
   */
  public GeoCoordinates deriveCoordinates(TransactionRequest request) {
    ...
    String cacheKey =
        String.format(
            "%s|%s|%s", countryCode, state != null ? state : "", city != null ? city : "");

    return coordinatesCache.computeIfAbsent(
        cacheKey, k -> lookupCoordinates(countryCode, state, city));
  }
  ...
  private GeoCoordinates lookupCoordinates(String countryCode, String state, String city) {
    try {
      ...
      if (city != null && !city.isBlank()) {
        Optional<GeoReference> exact =
            geoReferenceRepository.findByLocation(countryCode, state, normalizeCity(city));
        if (exact.isPresent()) {
        ...
        List<GeoReference> byCity =
            geoReferenceRepository.findByCityNameIgnoreCase(normalizeCity(city));
        if (!byCity.isEmpty()) {
      ...
      if (state != null && !state.isBlank()) {
        List<GeoReference> byState =
            geoReferenceRepository.findByCountryCodeAndStateCode(countryCode, state);
        if (!byState.isEmpty()) {
      ...
      log.debug(
          "Nenhuma referência geográfica encontrada para: country={}, state={}, city={}",
          countryCode,
          state,
          city);
      return GeoCoordinates.notFound();
  ...
  private String normalizeCity(String city) {
    ...
    return city.toUpperCase().replaceAll("[^A-Z0-9 ]", "").trim();
  }
  ...
  public GeoResult evaluateDistanceLessThan(
      TransactionRequest request, double targetLat, double targetLon, double thresholdKm) {
    ...
    double distance =
        haversineDistance(coords.getLatitude(), coords.getLongitude(), targetLat, targetLon);
    boolean result = distance < thresholdKm;

    log.debug(
        "GEO_DISTANCE_LT: ({},{}) -> ({},{}) = {:.2f}km < {:.2f}km = {}",
        coords.getLatitude(),
        coords.getLongitude(),
        targetLat,
        targetLon,
  ...
  public GeoResult evaluateInPolygon(TransactionRequest request, String polygonName) {
    ...
    try {
      ...
      if (!isInBoundingBox(coords.getLatitude(), coords.getLongitude(), polygon)) {
        return GeoResult.builder().success(true).result(false).point1(coords).build();
      }
      ...
      return GeoResult.builder().success(true).result(inside).point1(coords).build();
  ...
  private boolean isInBoundingBox(double lat, double lon, GeoPolygon polygon) {
    if (polygon.getMinLat() == null
        || polygon.getMaxLat() == null
        || polygon.getMinLon() == null
        || polygon.getMaxLon() == null) {
  ...
  private List<double[]> parsePolygonPoints(GeoPolygon polygon) {
    ...
    return polygonCache.computeIfAbsent(
        cacheKey,
        k -> {
          try {
            List<List<Double>> rawPoints =
                objectMapper.readValue(
                    polygon.getPolygonPoints(), new TypeReference<List<List<Double>>>() {});

            return rawPoints.stream().map(p -> new double[] {p.get(0), p.get(1)}).toList();
          } catch (Exception e) {
