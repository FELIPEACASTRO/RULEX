 ...
 *
 * <p>Baseado em pesquisa acadêmica (PESQUISA_REGRAS_DURAS_EFICIENTES.md): - Ordenação inteligente
 * pode reduzir latência em até 60% - Short-circuit evaluation com regras mais eficientes primeiro -
 * Balanceamento entre custo computacional e taxa de hit
 *
 * <p>Algoritmo de ordenação: 1. Score = (hitRate / computationCost) * weight 2. Regras com maior
 * score executam primeiro 3. Permite short-circuit quando classificação final é determinada 4.
 * Recalcula periodicamente baseado em métricas reais
 *
 * <p>Estratégias implementadas: - Cost-based ordering: regras baratas primeiro - Hit-rate ordering:
 * regras com mais hits primeiro - Hybrid ordering: balanceia custo e hit rate -
...
public class RuleOrderingService {
  ...
  private static final Map<RuleType, Double> BASE_COMPUTATION_COSTS =
      Map.of(
  ...
  /**
   * Retorna lista de regras na ordem otimizada para execução. Usa cache atualizado periodicamente.
   */
    ...
    if (optimizedOrderCache.isEmpty() || shouldRecompute()) {
      synchronized (this) {
        if (optimizedOrderCache.isEmpty() || shouldRecompute()) {
          ...
          log.info(
              "RuleOrdering: Cache de ordenação atualizado com {} regras",
  ...
  private List<RuleConfiguration> computeOptimalOrder() {
    ...
    return scoredRules.stream().map(sr -> sr.rule).collect(Collectors.toList());
  }

  /**
   * Calcula score ótimo para uma regra usando fórmula híbrida: Score = (hitRate * weight) /
   * (computationCost * (1 + avgLatencyMs/10))
   *
   * <p>Regras que detectam mais fraudes com menor custo têm maior score.
   */
  private double calculateOptimalScore(RuleConfiguration rule) {
    ...
    double classificationBoost =
        switch (rule.getClassification()) {
          case FRAUD -> 2.0;
          case SUSPICIOUS -> 1.5;
          default -> 1.0;
        };
    ...
    return rules.stream()
        .sorted(
            Comparator.comparingDouble(
                r -> BASE_COMPUTATION_COSTS.getOrDefault(r.getRuleType(), 5.0)))
        .collect(Collectors.toList());
  }
  ...
  /**
   * Ordenação por classificação (FRAUD > SUSPICIOUS > APPROVED). Permite short-circuit mais rápido.
   */
  private List<RuleConfiguration> orderByClassification(List<RuleConfiguration> rules) {
    Map<Object, Integer> classOrder =
        Map.of(
    ...
    return rules.stream()
        .sorted(Comparator.comparingInt(r -> classOrder.getOrDefault(r.getClassification(), 3)))
        .collect(Collectors.toList());
  }
  ...
  public OrderingEfficiencyReport getEfficiencyReport() {
    ...
    for (int i = 0; i < currentOrder.size(); i++) {
      ...
      if (metrics != null && metrics.totalExecutions > 0) {
        ...
        if (topPerformers.size() < 5) {
          topPerformers.add(
              RulePerformanceInfo.builder()
                  .ruleName(rule.getRuleName())
                  .position(i + 1)
                  .hitRate(
                      BigDecimal.valueOf(metrics.getHitRate() * 100)
                          .setScale(2, RoundingMode.HALF_UP))
                  .avgLatencyMs(
                      BigDecimal.valueOf(metrics.getAverageLatencyMs())
                          .setScale(2, RoundingMode.HALF_UP))
                  .score(
                      BigDecimal.valueOf(calculateOptimalScore(rule))
    ...
    return OrderingEfficiencyReport.builder()
        ...
        .weightedHitPosition(
            BigDecimal.valueOf(weightedHitPosition).setScale(4, RoundingMode.HALF_UP))
  ...
  private BigDecimal calculateLatencyReduction(List<RuleConfiguration> optimizedOrder) {
    ...
    for (int i = 0; i < optimizedOrder.size(); i++) {
      ...
      double costOptimized =
          BASE_COMPUTATION_COSTS.getOrDefault(optimizedOrder.get(i).getRuleType(), 5.0);
      double costRandom =
          BASE_COMPUTATION_COSTS.getOrDefault(randomOrder.get(i).getRuleType(), 5.0);
