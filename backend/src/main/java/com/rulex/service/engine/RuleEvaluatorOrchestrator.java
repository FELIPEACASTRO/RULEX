 ...
 * 
 * <p>Coordena a execucao de multiplos avaliadores e consolida resultados.
 * Permite adicionar novos avaliadores sem modificar codigo existente.
 * 
 * <p>Fluxo:
 * 1. Recebe transacao
 * 2. Determina quais avaliadores executar
 * 3. Executa avaliadores em paralelo ou sequencial
 * 4. Consolida resultados
 * 5. Retorna decisao final
 */
@Service
@Slf4j
        ...
        for (RuleEvaluator evaluator : evaluatorList) {
            for (RuleType type : evaluator.getSupportedRuleTypes()) {
                if (evaluators.containsKey(type)) {
                    log.warn("Tipo de regra {} ja tem avaliador registrado. Sobrescrevendo com {}",
                        type, evaluator.getName());
                }
                evaluators.put(type, evaluator);
            }
        }
        
        log.info("RuleEvaluatorOrchestrator inicializado com {} avaliadores para {} tipos de regra",
            evaluatorList.size(), evaluators.size());
    }
    ...
    public EvaluationResult evaluate(TransactionRequest transaction) {
        ...
        for (Map.Entry<RuleType, RuleEvaluator> entry : evaluators.entrySet()) {
            ...
            try {
                ...
                log.error("Erro ao executar avaliador {} para tipo {}: {}", 
                    evaluator.getName(), type, e.getMessage());
        ...
        if (anyTriggered) {
            return EvaluationResult.declined("Orchestrator", maxRiskScore, allTriggeredRules, executionTime);
        }
