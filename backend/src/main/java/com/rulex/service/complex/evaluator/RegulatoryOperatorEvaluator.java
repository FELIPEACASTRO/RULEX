 ...
 *
 * <p>NOTA: Operadores SCA_* que estavam como STUB foram movidos para StubOperatorEvaluator. Apenas
 * operadores com implementação real baseada em flags do payload ficam aqui.
 */
@Component
@Slf4j
public class RegulatoryOperatorEvaluator implements OperatorEvaluator {
  ...
  private boolean evaluateTraExemption(Map<String, Object> payload, RuleCondition condition) {
    ...
    log.debug(
        "SCA_EXEMPTION_TRA: riskScore={}, threshold={}, qualifies={}",
        riskScore,
        threshold,
        qualifies);
    return qualifies;
  ...
  private boolean evaluateLowValueExemption(TransactionRequest request) {
    ...
    log.debug(
        "SCA_EXEMPTION_LOW_VALUE: amount={}, limit={}, qualifies={}",
        amount,
        SCA_LOW_VALUE_LIMIT,
        qualifies);
    return qualifies;
  ...
  private boolean evaluateTrustedBeneficiary(Map<String, Object> payload) {
    ...
    log.debug(
        "SCA_EXEMPTION_TRUSTED_BENEFICIARY: trusted={}, whitelisted={}, qualifies={}",
        trusted,
        whitelisted,
        qualifies);
    return qualifies;
  ...
  private boolean evaluateRecurringExemption(Map<String, Object> payload) {
    ...
    log.debug(
        "SCA_EXEMPTION_RECURRING: recurring={}, subscription={}, qualifies={}",
        recurring,
        subscription,
        qualifies);
    return qualifies;
  ...
  private boolean evaluateCopNameMatch(Map<String, Object> payload) {
    ...
    log.debug(
        "PSD3_COP_NAME_MATCH: payee={}, holder={}, matches={}", payeeName, holderName, matches);
    return matches;
  }

  private boolean evaluateDoraIncidentSeverity(
      Map<String, Object> payload, RuleCondition condition) {
    ...
    if (severity != null) {
      ...
      log.debug(
          "DORA_INCIDENT_SEVERITY: severity={}, required={}, matches={}",
          severity,
          required,
          matches);
      return matches;
    ...
    if (level != null) {
      ...
      log.debug(
          "DORA_INCIDENT_SEVERITY: level={}, required={}, matches={}",
          level,
          requiredLevel,
          matches);
      return matches;
  ...
  private boolean evaluateGdprRetention(Map<String, Object> payload, RuleCondition condition) {
    ...
    int maxRetention =
        configuredRetention != null
            ? configuredRetention
            : parseInt(condition.getValueSingle(), 2555);

    boolean withinRetention = dataAgeDays <= maxRetention;
    log.debug(
        "GDPR_DATA_RETENTION: ageDays={}, maxRetention={}, withinRetention={}",
        dataAgeDays,
        maxRetention,
        withinRetention);
