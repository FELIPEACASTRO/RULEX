...
/**
 * Evaluator para operadores sincronizados do banco de dados. Implementa os 50 operadores que
 * estavam no DB mas não no backend.
 */
@Component
@Slf4j
public class DatabaseSyncOperatorEvaluator implements OperatorEvaluator {
  ...
  public boolean evaluate(RuleCondition condition, EvaluationContext context) {
    ...
    return switch (condition.getOperator()) {
      ...
      case COUNT_DISTINCT_COUNTRIES_LAST_N_DAYS ->
          evaluateCountDistinctCountriesLastNDays(fieldValue, conditionValue);
  ...
  private boolean evaluateAnd(
      Object fieldValue, String conditionValue, Map<String, Object> context) {
  ...
  private boolean evaluateNand(
      Object fieldValue, String conditionValue, Map<String, Object> context) {
    return !evaluateAnd(fieldValue, conditionValue, context);
  }

  private boolean evaluateNor(
      Object fieldValue, String conditionValue, Map<String, Object> context) {
  ...
  private boolean evaluateNotInList(
      Object fieldValue, java.util.List<String> valueArray, Map<String, Object> context) {
  ...
  private boolean evaluateVelocityAnomaly(
      Object fieldValue, String conditionValue, Map<String, Object> context) {
    try {
      double velocity = Double.parseDouble(fieldValue.toString());
      double avgVelocity =
          context != null && context.containsKey("avgVelocity")
              ? Double.parseDouble(context.get("avgVelocity").toString())
              : 0;
      double stdDev =
          context != null && context.containsKey("velocityStdDev")
              ? Double.parseDouble(context.get("velocityStdDev").toString())
              : 1;
  ...
  private boolean evaluateAmountAnomaly(
      Object fieldValue, String conditionValue, Map<String, Object> context) {
    try {
      double amount = Double.parseDouble(fieldValue.toString());
      double avgAmount =
          context != null && context.containsKey("avgAmount")
              ? Double.parseDouble(context.get("avgAmount").toString())
              : 0;
      double stdDev =
          context != null && context.containsKey("amountStdDev")
              ? Double.parseDouble(context.get("amountStdDev").toString())
              : 1;
  ...
  private boolean evaluateCountDistinctCountriesLastNDays(
      Object fieldValue, String conditionValue) {
    try {
