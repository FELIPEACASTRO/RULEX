 ...
 *
 * <ul>
 *   <li>ARRAY_CONTAINS, ARRAY_NOT_CONTAINS - elemento em array
 *   <li>ARRAY_SIZE_EQ, ARRAY_SIZE_GT, ARRAY_SIZE_LT - tamanho de array
 *   <li>ARRAY_ALL_MATCH, ARRAY_ANY_MATCH - predicados em array
 *   <li>MOD_EQ - módulo
 *   <li>MATH_ABS_LT, MATH_ABS_GT - valor absoluto
 *   <li>PERCENTAGE_OF_LT, PERCENTAGE_OF_GT - percentual
 * </ul>
 */
@Component
@Slf4j
public class ArrayMathOperatorEvaluator implements OperatorEvaluator {
  ...
  public boolean evaluate(RuleCondition condition, EvaluationContext context) {
    ...
    return switch (op) {
      ...
      case MOD_NEQ ->
          !evaluateModEq(fieldValue, condition.getValueSingle(), condition.getValueMin());
      default -> false;
    };
  }
  ...
  private boolean evaluateArrayContains(Object fieldValue, String element) {
    ...
    if (fieldValue instanceof Collection<?> collection) {
      return collection.stream().anyMatch(item -> element.equals(String.valueOf(item)));
    }
  ...
  private boolean evaluateArrayAllMatch(Object fieldValue, List<String> expectedValues) {
    ...
    for (String expected : expectedValues) {
      boolean found = collection.stream().anyMatch(item -> expected.equals(String.valueOf(item)));
  ...
  private boolean evaluatePercentageOf(
