 ...
 *
 * <ul>
 *   <li>NEO4J_WEAKLY_CONNECTED_COMPONENTS - componentes fracamente conectados
 *   <li>NEO4J_DEGREE_CENTRALITY - centralidade de grau
 *   <li>NEO4J_PAGERANK_FRAUD_SCORE - score de fraude via PageRank
 *   <li>NEO4J_LOUVAIN_COMMUNITY_DETECTION - detecção de comunidade
 *   <li>NEO4J_FRAUD_RING_DETECTION - detecção de anel de fraude
 *   <li>NEO4J_CIRCULAR_TRANSACTION_DETECTION - detecção de transação circular
 *   <li>NEO4J_BETWEENNESS_CENTRALITY_MULE - centralidade para mules
 *   <li>NEO4J_TRIANGLE_COUNT_COLLUSION - contagem de triângulos
 * </ul>
 */
@Component
...
public class GraphOperatorEvaluator implements OperatorEvaluator {
    ...
    try {
      return switch (op) {
        ...
        case NEO4J_PAIRWISE_SIMILARITY_PII ->
            evaluatePairwiseSimilarity(accountId, condition, context);
        ...
        case NEO4J_CIRCULAR_TRANSACTION_DETECTION ->
            evaluateCircularTransactions(accountId, condition);
        ...
        case NEO4J_BETWEENNESS_CENTRALITY_MULE ->
            evaluateBetweennessCentrality(accountId, condition);
        ...
        case NEO4J_NODE_SIMILARITY_SYNTHETIC_ID ->
            evaluateNodeSimilarity(accountId, condition, context);
        case NEO4J_GRAPH_EMBEDDING_FRAUD_PREDICTION ->
            evaluateFraudProbability(accountId, condition);
  ...
  private boolean evaluatePairwiseSimilarity(
      String accountId, RuleCondition condition, EvaluationContext context) {
  ...
  private boolean evaluateNodeSimilarity(
      String accountId, RuleCondition condition, EvaluationContext context) {
    String otherAccountId = condition.getValueFieldRef();
    if (otherAccountId == null) {
      Map<String, Object> payload = context.getPayload();
  ...
  private boolean evaluateTemporalMotif(String accountId, RuleCondition condition) {
    ...
    log.debug(
        "NEO4J_TEMPORAL_MOTIF: pattern={}, count={}, threshold={}", motifPattern, count, threshold);
