 ...
 *
 * <ul>
 *   <li>Template method para avaliação com tratamento de exceções
 *   <li>Métodos utilitários para extração de valores
 *   <li>Logging padronizado
...
public abstract class AbstractOperatorEvaluator implements OperatorEvaluator {
  ...
  public final boolean evaluate(RuleCondition condition, EvaluationContext context) {
    ...
    try {
      ...
      if (log.isDebugEnabled()) {
        ...
        log.debug(
            "[{}] Operador {} em campo '{}': resultado={}, tempo={}µs",
            getCategory(),
            operator,
            fieldName,
            result,
      ...
      log.error(
          "[{}] Erro de avaliação no operador {}: {}", getCategory(), operator, e.getMessage());
      throw e;

    } catch (IllegalArgumentException e) {
      // Argumentos inválidos - log e retorna false
      log.warn(
          "[{}] Argumento inválido no operador {} para campo '{}': {}",
          getCategory(),
          operator,
          fieldName,
          e.getMessage());
      ...
      log.debug(
          "[{}] Valor nulo ao avaliar operador {} no campo '{}': {}",
          getCategory(),
          operator,
          fieldName,
          e.getMessage());
      ...
      log.error(
          "[{}] Erro inesperado ao avaliar operador {} no campo '{}': {} - {}",
          getCategory(),
          operator,
          fieldName,
  ...
  protected double[] parseThresholdAndWindow(
