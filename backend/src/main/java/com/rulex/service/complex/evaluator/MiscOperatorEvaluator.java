 ...
 *
 * <ul>
 *   <li>IS_VOIP - número é VoIP
 *   <li>IS_CRYPTO_RANSOM_AMOUNT - valor típico de ransomware
 *   <li>IS_IMPOSSIBLE_COMBINATION - combinação impossível de dados
 *   <li>IS_HOLIDAY - data é feriado
 *   <li>HAS_FAILED_3DS_LAST_N_MINUTES - falha 3DS recente
 *   <li>HAS_INCOMING_TRANSFER_LAST_N_HOURS - transferência recebida
 *   <li>GTE_PERCENT_OF_LAST_INCOMING - percentual da última entrada
 *   <li>OUTFLOW_RATE_LAST_N_DAYS - taxa de saída
 *   <li>PIX_KEY_CHANGED_LAST_N_DAYS - chave PIX alterada
 *   <li>NAME_SIMILARITY_LT - similaridade de nome menor que
 *   <li>NAME_TRANSLITERATION_MATCH - match de transliteração
 * </ul>
 */
@Component
@Slf4j
public class MiscOperatorEvaluator implements OperatorEvaluator {
  ...
  private static final Set<String> VOIP_PREFIXES =
      Set.of("0300", "0303", "0500", "0800", "0900", "4000", "4003", "4004");

  // Valores típicos de ransomware em BTC (convertidos para BRL aproximado)
  private static final Set<BigDecimal> RANSOM_AMOUNTS =
      Set.of(
          new BigDecimal("500"),
          new BigDecimal("1000"),
          new BigDecimal("2000"),
  ...
  private static final Set<String> BRAZILIAN_HOLIDAYS =
      Set.of(
          "01-01", // Ano Novo
          "04-21", // Tiradentes
  ...
  private boolean evaluateIsImpossibleCombination(
    ...
    if (cardCountry != null && ipCountry != null && !cardCountry.equalsIgnoreCase(ipCountry)) {
      ...
      if (!getBooleanValue(payload, "isTraveling") && !getBooleanValue(payload, "knownTraveler")) {
        log.debug(
            "IS_IMPOSSIBLE_COMBINATION: cardCountry={} != ipCountry={}", cardCountry, ipCountry);
        return true;
      }
    }
  ...
  private boolean evaluateHasIncomingTransfer(RuleCondition condition, EvaluationContext context) {
    ...
    // Verificar flag direta
    if (getBooleanValue(payload, "hasIncomingTransfer")
        || getBooleanValue(payload, "has_incoming_transfer")) {
      return true;
    }
  ...
  private boolean evaluateGtePercentOfLastIncoming(
    ...
    // Calcular percentual
    BigDecimal percent =
        current.divide(incoming, 4, java.math.RoundingMode.HALF_UP).multiply(new BigDecimal("100"));
