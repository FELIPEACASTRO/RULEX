 ...
 *
 * <ul>
 *   <li>VELOCITY_COUNT_GT, VELOCITY_COUNT_LT - contagem de transações
 *   <li>VELOCITY_SUM_GT, VELOCITY_SUM_LT - soma de valores
 *   <li>VELOCITY_AVG_GT, VELOCITY_AVG_LT - média de valores
 *   <li>VELOCITY_DISTINCT_GT, VELOCITY_DISTINCT_LT - contagem distinta
 *   <li>COUNT_LAST_N_HOURS, SUM_LAST_N_DAYS, AVG_LAST_N_DAYS
 *   <li>COUNT_DISTINCT_* - contagens distintas por período
 * </ul>
 */
@Component
...
public class VelocityOperatorEvaluator implements OperatorEvaluator {
    ...
    try {
      return switch (op) {
        ...
        case COUNT_DISTINCT_MERCHANTS_LAST_N_DAYS ->
            evaluateCountDistinctMerchantsLastNDays(keyValue, condition, context);
        case COUNT_DISTINCT_MERCHANTS_LAST_N_HOURS ->
            evaluateCountDistinctMerchantsLastNHours(keyValue, condition, context);
        ...
        case COUNT_DISTINCT_COUNTRIES_LAST_N_HOURS ->
            evaluateCountDistinctCountriesLastNHours(keyValue, condition, context);
        default -> false;
      };
    } catch (Exception e) {
  ...
  private boolean evaluateVelocityCountGt(
      String keyValue, RuleCondition condition, EvaluationContext context) {
  ...
  private boolean evaluateVelocityCountLt(
      String keyValue, RuleCondition condition, EvaluationContext context) {
    TransactionRequest request = context.getTransactionRequest();
    if (request == null) return false;
  ...
  private boolean evaluateVelocitySumLt(
      String keyValue, RuleCondition condition, EvaluationContext context) {
    TransactionRequest request = context.getTransactionRequest();
    if (request == null) return false;
  ...
  private boolean evaluateVelocityAvgGt(
      String keyValue, RuleCondition condition, EvaluationContext context) {
    TransactionRequest request = context.getTransactionRequest();
    if (request == null) return false;
  ...
  private boolean evaluateVelocityAvgLt(
      String keyValue, RuleCondition condition, EvaluationContext context) {
    TransactionRequest request = context.getTransactionRequest();
    if (request == null) return false;
  ...
  private boolean evaluateVelocityDistinctGt(
      String keyValue, RuleCondition condition, EvaluationContext context) {
    TransactionRequest request = context.getTransactionRequest();
    if (request == null) return false;
    ...
    BigDecimal count =
        velocityService.getAggregation(
            request, keyType, window, AggregationType.DISTINCT_MERCHANTS);
  ...
  private boolean evaluateVelocityDistinctLt(
      String keyValue, RuleCondition condition, EvaluationContext context) {
    TransactionRequest request = context.getTransactionRequest();
    if (request == null) return false;
  ...
  private boolean evaluateSumLastNDays(
      String keyValue, RuleCondition condition, EvaluationContext context) {
    TransactionRequest request = context.getTransactionRequest();
    if (request == null) return false;
    ...
    BigDecimal threshold =
        tw.threshold != null ? tw.threshold : parseBigDecimalThreshold(condition.getValueSingle());
    log.debug("SUM_LAST_N_DAYS: days={}, sum={}, threshold={}", days, sum, threshold);
    return sum.compareTo(threshold) > 0;
  }

  private boolean evaluateSumLastNHours(
      String keyValue, RuleCondition condition, EvaluationContext context) {
    TransactionRequest request = context.getTransactionRequest();
    if (request == null) return false;
  ...
  private boolean evaluateCountLastNHours(
      String keyValue, RuleCondition condition, EvaluationContext context) {
    TransactionRequest request = context.getTransactionRequest();
    if (request == null) return false;
    ...
    long threshold =
        tw.threshold != null
            ? tw.threshold.longValue()
            : parseThreshold(condition.getValueSingle());
    log.debug("COUNT_LAST_N_HOURS: hours={}, count={}, threshold={}", hours, count, threshold);
    return count.longValue() > threshold;
  }

  private boolean evaluateAvgLastNDays(
      String keyValue, RuleCondition condition, EvaluationContext context) {
    TransactionRequest request = context.getTransactionRequest();
    if (request == null) return false;
  ...
  private boolean evaluateCountDistinctMerchantsLastNDays(
    ...
    long threshold =
        tw.threshold != null
            ...
            : parseThreshold(condition.getValueSingle());
    log.debug(
        "COUNT_DISTINCT_MERCHANTS_LAST_N_DAYS: days={}, count={}, threshold={}",
        days,
        count,
        threshold);
    return count.longValue() > threshold;
  ...
  private boolean evaluateCountDistinctMerchantsLastNHours(
    ...
    long threshold =
        tw.threshold != null
            ...
            : parseThreshold(condition.getValueSingle());
    log.debug(
        "COUNT_DISTINCT_MERCHANTS_LAST_N_HOURS: hours={}, count={}, threshold={}",
        hours,
        count,
        threshold);
    return count.longValue() > threshold;
  ...
  private boolean evaluateMinAmountLastNDays(
      String keyValue, RuleCondition condition, EvaluationContext context) {
    TransactionRequest request = context.getTransactionRequest();
    if (request == null) return false;
  ...
  private boolean evaluateCountDistinctCountriesLastNHours(
    ...
    BigDecimal count =
        velocityService.getAggregation(
            request, keyType, window, AggregationType.DISTINCT_COUNTRIES);
    ...
    long threshold =
        tw.threshold != null
            ...
            : parseThreshold(condition.getValueSingle());
    log.debug(
        "COUNT_DISTINCT_COUNTRIES_LAST_N_HOURS: hours={}, count={}, threshold={}",
        hours,
        count,
        threshold);
    return count.longValue() > threshold;
