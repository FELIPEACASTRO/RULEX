...
public class RuleValidationService {
     ...
     */
    private void validateCondition(ConditionDTO condition, List<String> errors, List<String> warnings) {
        if (condition == null) {
            return;
        }
    ...
    public ValidationResult validateCondition(RuleCondition condition) {
        ...
        if (operator != null && UNSUPPORTED_OPERATORS.contains(operator.name())) {
            errors.add(String.format(
                "Operador '%s' não está implementado",
                operator.name()
            ));
        }
        ...
        if (valueType != null && UNSUPPORTED_VALUE_TYPES.contains(valueType.name())) {
            errors.add(String.format(
                "Tipo de valor '%s' não está implementado",
                valueType.name()
            ));
        }
