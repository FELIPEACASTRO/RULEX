 ...
 *
 * <p>Mapeia cada ConditionOperator para seu avaliador correspondente. Operadores sem avaliador
 * registrado lançam UnsupportedOperatorException.
 */
@Component
...
public class OperatorEvaluatorRegistry {
  ...
  private final Map<ConditionOperator, OperatorEvaluator> registry =
      new EnumMap<>(ConditionOperator.class);
  private final Map<ConditionOperator, OperatorStatus> statusMap =
      new EnumMap<>(ConditionOperator.class);
  ...
  public void init() {
    ...
    for (OperatorEvaluator evaluator : evaluators) {
      ...
      for (ConditionOperator op : supported) {
        if (registry.containsKey(op)) {
          log.warn(
              "Operador {} já registrado por {}. Sobrescrevendo com {}",
              op,
              registry.get(op).getClass().getSimpleName(),
              evaluatorName);
        }
    ...
    for (ConditionOperator op : ConditionOperator.values()) {
      if (!registry.containsKey(op)) {
        log.error(
            "OPERADOR SEM AVALIADOR: {}. Adicione ao StubOperatorEvaluator ou implemente.", op);
        missing++;
      }
    }
    ...
    int planned =
        (int) statusMap.values().stream().filter(s -> s == OperatorStatus.PLANNED).count();
  ...
  public OperatorEvaluator getEvaluator(ConditionOperator operator) {
    ...
    if (evaluator == null) {
      throw new UnsupportedOperatorException(
          operator, "Nenhum avaliador registrado para este operador.");
    }
    return evaluator;
  }
