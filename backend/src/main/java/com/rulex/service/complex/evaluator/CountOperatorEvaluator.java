 ...
 * <ul>
 *   <li>COUNT_LAST_N_DAYS - contagem nos últimos N dias</li>
 *   <li>COUNT_DISTINCT_ACCOUNTS - contagem de contas distintas</li>
 *   <li>COUNT_DISTINCT_INSTRUMENTS_LAST_N_DAYS - instrumentos distintos</li>
 *   <li>COUNT_DISTINCT_PANS_LAST_N_HOURS - PANs distintos</li>
 *   <li>COUNT_DISTINCT_PAYERS_LAST_N_DAYS - pagadores distintos</li>
 *   <li>COUNT_DISTINCT_USER_AGENTS_LAST_N_HOURS - user agents distintos</li>
 *   <li>COUNT_CRYPTO_TXN_LAST_N_DAYS - transações crypto</li>
 *   <li>COUNT_MFA_ABANDONMENTS - abandonos de MFA</li>
 *   <li>COUNT_MFA_DENIALS_LAST_N_HOURS - negações de MFA</li>
 *   <li>COUNT_FAILURES_LAST_N_HOURS - falhas recentes</li>
 *   <li>COUNT_UNIQUE_BENEFICIARIES_LAST_N_DAYS - beneficiários únicos</li>
 *   <li>COUNT_UNIQUE_IPS_LAST_N_HOURS - IPs únicos</li>
 *   <li>COUNT_DISTINCT_MERCHANTS_LAST_N_HOURS - merchants distintos por hora</li>
 * </ul>
 */
@Component
...
public class CountOperatorEvaluator implements OperatorEvaluator {
    ...
    public boolean evaluate(RuleCondition condition, EvaluationContext context) {
        ...
        return switch (op) {
            ...
            case COUNT_DISTINCT_INSTRUMENTS_LAST_N_DAYS -> evaluateCountDistinctInstruments(condition, context);
            ...
            case COUNT_DISTINCT_USER_AGENTS_LAST_N_HOURS -> evaluateCountDistinctUserAgents(condition, context);
            ...
            case COUNT_UNIQUE_BENEFICIARIES_LAST_N_DAYS -> evaluateCountUniqueBeneficiaries(condition, context);
            case COUNT_UNIQUE_IPS_LAST_N_HOURS -> evaluateCountUniqueIps(condition, context);
            case COUNT_DISTINCT_MERCHANTS_LAST_N_HOURS -> evaluateCountDistinctMerchantsHours(condition, context);
            default -> false;
        };
    }
     ...
     */
    private boolean evaluateCountDistinctInstruments(RuleCondition condition, EvaluationContext context) {
        TransactionRequest request = context.getTransactionRequest();
        if (request == null) {
        ...
        BigDecimal count = velocityService.getAggregation(request, KeyType.CUSTOMER_ID, window, AggregationType.DISTINCT_MERCHANTS);
        long threshold = parseLongSafe(condition.getValueSingle(), 0);
     ...
     */
    private boolean evaluateCountDistinctUserAgents(RuleCondition condition, EvaluationContext context) {
     ...
     */
    private boolean evaluateCountUniqueBeneficiaries(RuleCondition condition, EvaluationContext context) {
     ...
     */
    private boolean evaluateCountDistinctMerchantsHours(RuleCondition condition, EvaluationContext context) {
        TransactionRequest request = context.getTransactionRequest();
        if (request == null) {
        ...
        BigDecimal count = velocityService.getAggregation(request, KeyType.PAN, window, AggregationType.DISTINCT_MERCHANTS);
        long threshold = parseLongSafe(condition.getValueSingle(), 0);
    ...
    private boolean evaluateFromPayload(RuleCondition condition, EvaluationContext context, String defaultKey) {
