...
/**
 * Avaliador para operadores de padrões de transação.
 * Detecta padrões suspeitos como splitting, layering, structuring, etc.
 */
@Component
@Slf4j
public class TransactionPatternOperatorEvaluator implements OperatorEvaluator {
    ...
    public boolean evaluate(RuleCondition condition, EvaluationContext context) {
        ...
        return switch (op) {
            case TRANSACTION_COUNT_PER_CARD_HOUR -> evaluateCount(context, "txCountPerCardHour", condition, 10);
            case TRANSACTION_COUNT_PER_CUSTOMER_HOUR -> evaluateCount(context, "txCountPerCustomerHour", condition, 15);
            case TRANSACTION_COUNT_PER_DEVICE_DAY -> evaluateCount(context, "txCountPerDeviceDay", condition, 50);
            case TRANSACTION_COUNT_PER_IP_HOUR -> evaluateCount(context, "txCountPerIpHour", condition, 20);
            case TRANSACTION_COUNT_PER_MERCHANT_HOUR -> evaluateCount(context, "txCountPerMerchantHour", condition, 100);
            ...
            case TRANSACTION_ATTEMPT_COUNT_PER_CARD -> evaluateCount(context, "txAttemptCountPerCard", condition, 5);
    ...
    private boolean evaluateCount(EvaluationContext context, String key, RuleCondition condition, int defaultThreshold) {
        Map<String, Object> payload = context.getPayload();
        if (payload == null) return false;
        Object value = payload.get(key);
        if (value == null) return false;
