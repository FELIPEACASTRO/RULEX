...
public class ComplexRuleService {
    ...
    public ConditionGroupDTO getConditionGroup(UUID ruleVersionId) {
        return conditionGroupRepository.findByRuleVersionIdAndParentGroupIdIsNull(ruleVersionId)
                .map(mapper::toDTO)
                .orElse(null);
    }
    ...
    public List<ExpressionDTO> saveExpressions(UUID ruleVersionId, List<ExpressionDTO> dtos) {
        ...
        List<RuleExpression> expressions = dtos.stream()
                .map(dto -> mapper.toEntity(dto, ruleVersionId))
     ...
     */
    @Transactional
    public List<ContextVariableDTO> saveContextVariables(UUID ruleVersionId, List<ContextVariableDTO> dtos) {
        log.info("Salvando {} variáveis de contexto para ruleVersionId: {}", dtos.size(), ruleVersionId);

        // Deletar variáveis existentes
        contextVariableRepository.deleteByRuleVersionId(ruleVersionId);

        // Criar novas variáveis
        List<RuleContextVariable> variables = dtos.stream()
                .map(dto -> mapper.toEntity(dto, ruleVersionId))
                .collect(Collectors.toList());
