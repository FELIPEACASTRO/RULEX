 ...
 *
 * <ul>
 *   <li>ACCOUNT_AGE_LT_MINUTES - conta criada há menos de N minutos
 *   <li>ACCOUNT_LINK_DEPTH - profundidade de links da conta
 *   <li>ACCOUNT_TAKEOVER_PATTERN - padrão de tomada de conta
 *   <li>DAYS_SINCE_LAST_ACTIVITY - dias desde última atividade
 *   <li>CHARGEBACK_RATE_GT - taxa de chargeback maior que
 *   <li>IS_NEW - conta é nova
 *   <li>IS_FIRST - primeira transação
 * </ul>
 */
@Component
@Slf4j
public class AccountOperatorEvaluator implements OperatorEvaluator {
  ...
  private boolean evaluateIsFirst(RuleCondition condition, EvaluationContext context) {
    ...
    // Verificar flag direta
    if (getBooleanValue(payload, "isFirstTransaction")
        || getBooleanValue(payload, "is_first_transaction")) {
      return true;
    }
