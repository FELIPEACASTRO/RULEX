...
/**
 * Avaliador estendido para operadores diversos.
 * Cobre velocidade, tempo, campos, tipos de dados e outros.
 */
@Component
@Slf4j
public class ExtendedOperatorEvaluator implements OperatorEvaluator {
    ...
    private boolean evaluateVelocityOperator(ConditionOperator op, RuleCondition condition, EvaluationContext context) {
    ...
    private boolean evaluateTimeOperator(ConditionOperator op, RuleCondition condition, EvaluationContext context) {
        Map<String, Object> payload = context.getPayload();
        if (payload == null) return false;
    ...
    private boolean evaluateFieldOperator(ConditionOperator op, RuleCondition condition, EvaluationContext context) {
        Map<String, Object> payload = context.getPayload();
        if (payload == null) return false;
        ...
        return switch (op) {
            ...
            case PERCENTAGE_OF_FIELD -> {
                ...
                BigDecimal pct = v1.divide(v2, 4, java.math.RoundingMode.HALF_UP).multiply(new BigDecimal("100"));
    ...
    private boolean evaluateSpecificOperator(ConditionOperator op, RuleCondition condition, EvaluationContext context) {
        Map<String, Object> payload = context.getPayload();
        if (payload == null) return false;

        return switch (op) {
            case BENEFICIARY_CONCENTRATION -> evaluateThreshold(payload, "beneficiaryConcentration", condition, 80);
            ...
            case DAILY_LIMIT_PROXIMITY -> evaluateThreshold(payload, "dailyLimitProximity", condition, 90);
            case WEEKLY_LIMIT_PROXIMITY -> evaluateThreshold(payload, "weeklyLimitProximity", condition, 90);
            ...
            case SUM_BY_CHANNEL_LAST_N_DAYS -> evaluateThreshold(payload, "sumByChannelLastNDays", condition, 0);
            ...
            case IN_CUSTOMER_CHARGEBACK_MERCHANTS -> evaluateBoolean(payload, "inCustomerChargebackMerchants");
            ...
            case PEER_GROUP_DEVIATION_SCORE -> evaluateThreshold(payload, "peerGroupDeviationScore", condition, 2);
            ...
            case UNIQUE_CARD_COUNT_PER_IP_HOUR -> evaluateCountThreshold(payload, "uniqueCardCountPerIpHour", condition, 10);
            case UNIQUE_MERCHANT_COUNT_PER_CARD_DAY -> evaluateCountThreshold(payload, "uniqueMerchantCountPerCardDay", condition, 20);
    ...
    private boolean evaluateThreshold(Map<String, Object> payload, String key, RuleCondition condition, double defaultThreshold) {
        Object value = payload.get(key);
        if (value == null) return false;
        BigDecimal v = parseBigDecimal(String.valueOf(value), BigDecimal.ZERO);
    ...
    private boolean evaluateThresholdLessThan(Map<String, Object> payload, String key, RuleCondition condition, int defaultThreshold) {
        Object value = payload.get(key);
        if (value == null) return false;
    ...
    private boolean evaluateCountThreshold(Map<String, Object> payload, String key, RuleCondition condition, int defaultThreshold) {
        Object value = payload.get(key);
        if (value == null) return false;
