...
/**
 * Avaliador para operadores relacionados a valores/amounts. Implementa detecção de anomalias,
 * spikes, variância e padrões de valores.
 */
@Component
@Slf4j
public class AmountOperatorEvaluator implements OperatorEvaluator {
  ...
  private boolean evaluateAmountDeviationFromAvg(
    ...
    log.debug(
        "AMOUNT_DEVIATION_FROM_AVG: amount={}, avg={}, deviation={}%, threshold={}%",
  ...
  private boolean evaluatePercentageOfBalanceGt(
      RuleCondition condition, EvaluationContext context) {
    BigDecimal amount = getAmount(context);
    if (amount == null) return false;
    ...
    BigDecimal percentage =
        amount.divide(balance, 4, RoundingMode.HALF_UP).multiply(new BigDecimal("100"));
