...
public class BehavioralOperatorEvaluator implements OperatorEvaluator {
    ...
    public boolean evaluate(RuleCondition condition, EvaluationContext context) {
        ...
        return switch (op) {
            case ADAPTIVE_BEHAVIORAL_ANALYTICS -> evaluateScore(context, "adaptiveBehavioralScore", condition, 0.7);
            case ADAPTIVE_PARAMETRIC_THRESHOLD -> evaluateBoolean(context, "adaptiveThresholdExceeded");
            case BEHAVIORAL_BASELINE_DEVIATION -> evaluateScore(context, "behavioralBaselineDeviation", condition, 2.0);
            ...
            case FACE_TO_ID_PHOTO_MATCHING -> evaluateScoreLessThan(context, "faceToIdPhotoMatch", condition, 0.8);
    ...
    private boolean evaluateScoreLessThan(EvaluationContext context, String key, RuleCondition condition, double defaultThreshold) {
        Map<String, Object> payload = context.getPayload();
        if (payload == null) return false;
        Object value = payload.get(key);
        if (value == null) return false;
