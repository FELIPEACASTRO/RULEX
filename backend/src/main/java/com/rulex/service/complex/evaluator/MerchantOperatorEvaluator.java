...
/**
 * Avaliador para operadores relacionados a merchants/comerciantes. Implementa verificações de
 * reputação, padrões e anomalias de merchants.
 */
@Component
@Slf4j
public class MerchantOperatorEvaluator implements OperatorEvaluator {
  ...
  private boolean evaluateMerchantAmountDistribution(
    ...
    Object distObj =
        getPayloadValue(context, "merchantAmountDistribution", "amountDistributionScore");
  ...
  private boolean evaluateMerchantCustomerConcentration(
      RuleCondition condition, EvaluationContext context) {
    Object concObj =
        getPayloadValue(context, "merchantCustomerConcentration", "customerConcentration");
  ...
  private boolean evaluateMerchantDormantReactivation(
    ...
    Object dormantObj =
        getPayloadValue(
            context, "merchantDormantReactivation", "isDormantReactivated", "dormantDays");
  ...
  private boolean evaluateMerchantGeographicSpread(
      RuleCondition condition, EvaluationContext context) {
    Object spreadObj =
        getPayloadValue(context, "merchantGeographicSpread", "geoSpread", "countryCount");
  ...
  private boolean evaluateMerchantFirstSeen(RuleCondition condition, EvaluationContext context) {
    Object firstSeenObj =
        getPayloadValue(context, "merchantFirstSeen", "isNewMerchant", "firstSeen");
