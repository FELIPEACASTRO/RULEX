  ...
  public boolean evaluate(RuleCondition condition, EvaluationContext context) {
    ...
    return switch (op) {
      case LLM_ADVERSARIAL_ATTACK_RESISTANCE ->
          evaluateScore(context, "llmAdversarialScore", condition, 0.7);
      case LLM_ANOMALY_EXPLANATION_GENERATION -> evaluateBoolean(context, "llmAnomalyDetected");
      case LLM_CHATBOT_FRAUD_DETECTION ->
          evaluateScore(context, "llmChatbotFraudScore", condition, 0.7);
      case LLM_DEEPFAKE_VOICE_DETECTION ->
          evaluateScore(context, "llmDeepfakeVoiceScore", condition, 0.7);
      case LLM_EMAIL_PHISHING_ANALYSIS ->
          evaluateScore(context, "llmEmailPhishingScore", condition, 0.6);
      case LLM_FRAUD_ALERT_PRIORITIZATION ->
          evaluateScore(context, "llmFraudAlertPriority", condition, 0.8);
      ...
      case LLM_MULTI_MODAL_FRAUD_DETECTION ->
          evaluateScore(context, "llmMultiModalFraudScore", condition, 0.7);
      case LLM_SOCIAL_ENGINEERING_CLASSIFICATION ->
          evaluateScore(context, "llmSocialEngineeringScore", condition, 0.6);
      case LLM_SYNTHETIC_IMAGE_DETECTION ->
          evaluateScore(context, "llmSyntheticImageScore", condition, 0.7);
      case LLM_TRANSACTION_DESCRIPTION_ANALYSIS ->
          evaluateScore(context, "llmTransactionDescriptionScore", condition, 0.6);
      default -> false;
    };
  }
