 ...
 *
 * <ul>
 *   <li>STRUCTURING_DETECTION - detecção de estruturação (smurfing)
 *   <li>LAYERING_PATTERN - padrão de camadas
 * </ul>
 */
@Component
@Slf4j
public class AmlFraudOperatorEvaluator implements OperatorEvaluator {
  ...
  private static final BigDecimal CTR_THRESHOLD =
      new BigDecimal("10000"); // Currency Transaction Report threshold
  ...
  private boolean evaluateStructuring(
    ...
    boolean inStructuringRange =
        amount.compareTo(lowerBound) >= 0 && amount.compareTo(CTR_THRESHOLD) < 0;
    ...
    boolean hasMultipleSimilar =
        (similarTxnCount != null && similarTxnCount >= threshold)
    ...
    log.debug(
        "STRUCTURING_DETECTION: amount={}, inRange={}, similarCount={}, round={}, isStructuring={}",
        amount,
        inStructuringRange,
        similarTxnCount,
        isRoundNumber,
  ...
  private boolean evaluateLayering(
      TransactionRequest request, Map<String, Object> payload, RuleCondition condition) {
    ...
    boolean hasMultipleHops =
        (hopCount != null && hopCount >= hopThreshold)
            || (intermediaryCount != null && intermediaryCount >= hopThreshold);

    // Verificar movimento rápido
    boolean isRapid =
        Boolean.TRUE.equals(rapidMovement)
    ...
    log.debug(
        "LAYERING_PATTERN: hops={}, rapid={}, crossBorder={}, isLayering={}",
        hopCount,
        isRapid,
        isCrossBorder,
        isLayering);
