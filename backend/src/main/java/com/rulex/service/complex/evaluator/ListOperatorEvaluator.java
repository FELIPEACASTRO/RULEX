 ...
 *
 * <ul>
 *   <li>IN_LIST - valor está em uma lista
 *   <li>NOT_IN_HISTORICAL - valor não está no histórico
 *   <li>NOT_IN_CUSTOMER_HISTORY - valor não está no histórico do cliente
 *   <li>NOT_IN_CUSTOMER_USUAL_HOURS - transação fora do horário usual
 *   <li>DOMAIN_IN_LIST - domínio está em lista
 *   <li>CONTAINS_SUSPICIOUS_KEYWORDS - contém palavras suspeitas
 * </ul>
 */
@Component
@Slf4j
public class ListOperatorEvaluator implements OperatorEvaluator {
  ...
  public boolean evaluate(RuleCondition condition, EvaluationContext context) {
    ...
    return switch (op) {
      case IN_LIST ->
          evaluateInList(fieldValue, condition.getValueArray(), condition.getCaseSensitive());
  ...
  /**
   * IN_LIST - verifica se o valor está em uma lista fornecida. Alias para IN do
   * BasicComparisonEvaluator.
   */
  ...
  /**
   * NOT_IN_HISTORICAL - verifica se o valor não está no histórico geral. Útil para detectar novos
   * merchants, novos países, etc.
   */
  private boolean evaluateNotInHistorical(
    ...
    if (history instanceof List<?> historyList) {
      String fieldStr = String.valueOf(fieldValue);
      return historyList.stream().map(String::valueOf).noneMatch(h -> h.equalsIgnoreCase(fieldStr));
    }
  ...
  private boolean evaluateNotInCustomerHistory(
      Object fieldValue, RuleCondition condition, EvaluationContext context) {
    if (fieldValue == null) {
      return true;
    }
  ...
  private boolean evaluateDomainInList(Object fieldValue, List<String> domains) {
    ...
    Set<String> domainList =
        (domains != null && !domains.isEmpty()) ? Set.copyOf(domains) : SUSPICIOUS_DOMAINS;

    return domainList.stream().anyMatch(d -> d.equalsIgnoreCase(domain));
  }
