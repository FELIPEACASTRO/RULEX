...
public class ComplexRuleCrudService {
    ...
    public List<ComplexRuleDTO> listAll() {
        return complexRuleRepository.findAll().stream()
                .map(this::toDTO)
    ...
    public ComplexRuleDTO getById(UUID id) {
        return complexRuleRepository.findById(id)
                .map(this::toDTO)
                .orElse(null);
    }
    ...
    public ComplexRuleDTO getByKey(String key) {
        return complexRuleRepository.findByKey(key)
                .map(this::toDTO)
                .orElse(null);
    }
    ...
    private ComplexRuleDTO toDTO(ComplexRule entity) {
        ComplexRuleDTO dto = ComplexRuleDTO.builder()
                .id(entity.getId())
                .key(entity.getKey())
        ...
        conditionGroupRepository.findRootByRuleVersionId(entity.getId())
                .ifPresent(group -> dto.setRootConditionGroup(mapper.toDTO(group)));

        // Carregar expressões
        dto.setExpressions(expressionRepository.findByRuleVersionId(entity.getId()).stream()
                .map(mapper::toDTO)
                .collect(Collectors.toList()));

        // Carregar variáveis de contexto
        dto.setContextVariables(contextVariableRepository.findByRuleVersionId(entity.getId()).stream()
                .map(mapper::toDTO)
                .collect(Collectors.toList()));

        // Carregar ações
        dto.setActions(actionRepository.findByRuleVersionIdOrderByExecutionOrder(entity.getId()).stream()
                .map(mapper::toDTO)
                .collect(Collectors.toList()));
    ...
    private ComplexRuleDTO.RuleStatusType parseStatus(String status) {
        try {
            return status != null ? ComplexRuleDTO.RuleStatusType.valueOf(status) : ComplexRuleDTO.RuleStatusType.DRAFT;
    ...
    private ComplexRuleDTO.DecisionType parseDecision(String decision) {
        try {
            return decision != null ? ComplexRuleDTO.DecisionType.valueOf(decision) : ComplexRuleDTO.DecisionType.APROVADO;
        } catch (IllegalArgumentException e) {
