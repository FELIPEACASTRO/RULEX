...
/**
 * Serviço de execução de regras complexas. Coordena a avaliação de regras com variáveis de
 * contexto, expressões e ações.
 */
@Service
...
public class ComplexRuleExecutionService {
  ...
  @Transactional
  public ComplexRuleExecutionResult executeRule(
      UUID ruleVersionId, Map<String, Object> payload, UUID decisionLogId) {
    ...
    try {
      ...
      if (evalResult.isMatched()) {
        List<RuleAction> actions =
            actionRepository.findByRuleVersionIdAndEnabledTrueOrderByPositionAsc(ruleVersionId);
  ...
  private Map<String, Object> prepareContextVariables(
      UUID ruleVersionId, Map<String, Object> payload) {
    Map<String, Object> variables = new HashMap<>();

    List<RuleContextVariable> contextVars =
        contextVariableRepository.findByRuleVersionId(ruleVersionId);
  ...
  private Object resolveContextVariable(
      RuleContextVariable var, Map<String, Object> payload, Map<String, Object> variables) {
  ...
  private Map<String, Object> evaluateExpressions(
      UUID ruleVersionId, Map<String, Object> payload, Map<String, Object> variables) {
    ...
    try {
      ...
      if (action.getConditionGroupId() != null) {
        RuleConditionGroup conditionGroup =
            conditionGroupRepository.findById(action.getConditionGroupId()).orElse(null);

        if (conditionGroup != null) {
          ComplexRuleEvaluator.EvaluationResult condResult =
              ruleEvaluator.evaluate(conditionGroup, context);
      ...
      switch (action.getActionType()) {
        ...
        case SET_SCORE -> {
          ...
          if (scoreValue != null) {
            ...
            int currentScore =
                variables.containsKey("riskScore")
        ...
        case SEND_NOTIFICATION -> {
          log.info(
              "Notificação seria enviada: canal={}, template={}",
              config.get("channel"),
              config.get("template"));
  ...
  public List<ComplexRuleExecutionResult> executeRules(
      List<UUID> ruleVersionIds, Map<String, Object> payload, UUID decisionLogId) {
