 ...
 * 
 * <p>Calcula agregações temporais (COUNT, SUM, AVG) sem alterar o payload de entrada.
 * As agregações são baseadas em:
 * - PAN (hash para privacidade)
 * - customerId
 * - merchantId
 * 
...
public class VelocityService {
    ...
    public void logTransaction(TransactionRequest request, String decision, Integer riskScore) {
        ...
        if (logRepository.findByExternalTransactionId(request.getExternalTransactionId()).isPresent()) {
            log.debug("Transação {} já registrada no log de velocidade", request.getExternalTransactionId());
            return;
        }

        try {
            VelocityTransactionLog logEntry = VelocityTransactionLog.builder()
        ...
        try {
            ...
            switch (keyType) {
                ...
                case CUSTOMER_ID -> {
                    ...
                    avg = count > 0 ? sum.divide(BigDecimal.valueOf(count), 2, RoundingMode.HALF_UP) : BigDecimal.ZERO;
                }
                case MERCHANT_ID -> {
    ...
    public long countPanTransactionsInHours(TransactionRequest request, int hours) {
        TimeWindow window = switch (hours) {
    ...
    public boolean isBurst(TransactionRequest request, int maxTransactions, int minutes) {
        TimeWindow window = switch (minutes) {
