 ...
 *
 * <ul>
 *   <li>Distance between transaction locations
 *   <li>Time elapsed between transactions
 *   <li>Calculated travel speed
 * </ul>
 *
 * <p>Example: If Card A transacts in São Paulo at 10:00 and in Tokyo at 10:30, the required speed
 * would be ~36,000 km/h - clearly impossible.
 *
 * <p>Thresholds:
 *
 * <ul>
 *   <li>Commercial flight: ~900 km/h max
 *   <li>Private jet: ~1,100 km/h max
 *   <li>Conservative threshold: 500 km/h (allows for some GPS variance)
 *   <li>Aggressive threshold: 200 km/h (land travel only)
 * </ul>
 *
 * <p>False positive mitigation:
 *
 * <ul>
 *   <li>Card-not-present transactions may show merchant location, not cardholder
 *   <li>VPN/proxy usage can mask true location
 *   <li>GPS variance can add ~50km error
 *   <li>Time zone differences in transaction timestamps
 * </ul>
 */
@Service
...
public class ImpossibleTravelService {
  ...
  public record TravelAnalysis(
    ...
    public static TravelAnalysis noHistory() {
      return new TravelAnalysis(
          TravelRisk.UNKNOWN, 0, 0, 0, null, null, List.of("NO_PREVIOUS_LOCATION"), 0);
    }
  }
   ...
   */
  public TravelAnalysis analyzeTravel(
      String panHash,
      double currentLat,
      double currentLon,
      String city,
    ...
    LocationHistory history =
        locationHistories.computeIfAbsent(panHash, k -> new LocationHistory());
    ...
    double distanceKm =
        calculateDistance(
            previousLocation.latitude(), previousLocation.longitude(), currentLat, currentLon);
    ...
    TravelRisk riskLevel =
        assessRisk(speedKmh, distanceKm, elapsed, previousLocation, currentLocation, riskFactors);
    ...
    log.debug(
        "Travel analysis for {}: {} km in {} min = {} km/h, risk={}",
        maskPan(panHash),
  ...
  /** Checks if a specific travel pattern is possible. Static utility method for one-off checks. */
  public static boolean isPossibleTravel(
      double fromLat, double fromLon, double toLat, double toLon, Duration elapsed) {
