 ...
 *
 * <p>Device fingerprinting creates a unique identifier for a device based on multiple attributes
 * that are difficult to forge. This helps detect:
 *
 * <ul>
 *   <li>Device hijacking: Same card used from completely different device
 *   <li>Device farming: Single device used with many different cards
 *   <li>Device spoofing: Attempts to mask device identity
 *   <li>New device risk: First-time device for a known card
 * </ul>
 *
 * <p>Fingerprint components (weighted by reliability):
 *
 * <ol>
 *   <li>Hardware signals: Screen resolution, platform, memory, CPU cores
 *   <li>Software signals: User agent, language, timezone
 *   <li>Browser signals: Plugins, fonts, canvas hash, WebGL hash
 *   <li>Network signals: IP prefix, connection type
 * </ol>
 *
 * <p>Similarity scoring uses Jaccard similarity and weighted component matching to handle partial
 * fingerprint matches (user updates browser, etc.).
 */
@Service
...
public class DeviceFingerprintService {
   ...
   */
  public FingerprintAnalysis analyzeFingerprint(
      String panHash, Map<String, Object> rawFingerprint) {
    ...
    RiskLevel riskLevel =
        calculateRiskLevel(
            isNewDevice, isDeviceFarm, isSpoofingAttempt, similarityScore, devicesForCard);
  ...
  public Map<String, Object> getDeviceRisk(String fingerprintHash) {
    ...
    return Map.of(
        ...
        cardCount,
        "isDeviceFarm",
        cardCount >= MAX_CARDS_PER_DEVICE,
        "riskLevel",
        cardCount >= MAX_CARDS_PER_DEVICE ? "HIGH" : cardCount > 3 ? "MEDIUM" : "LOW");
  }
  ...
  private DeviceFingerprint buildFingerprint(Map<String, Object> raw) {
    ...
    List<String> plugins =
        raw.get("plugins") instanceof List<?> ? (List<String>) raw.get("plugins") : List.of();
    @SuppressWarnings("unchecked")
    List<String> fonts =
        raw.get("fonts") instanceof List<?> ? (List<String>) raw.get("fonts") : List.of();
    ...
    String hash =
        generateFingerprintHash(
            userAgent, screenResolution, platform, canvasHash, webglHash, audioHash);
  ...
  private boolean detectSpoofing(
      DeviceFingerprint current, DeviceFingerprint known, List<String> riskFactors) {
    ...
    if (current.deviceMemory() != null && known.deviceMemory() != null) {
      if (Math.abs(current.deviceMemory() - known.deviceMemory()) > 8) {
        riskFactors.add(
            "MEMORY_INCONSISTENCY: " + known.deviceMemory() + " -> " + current.deviceMemory());
        spoofingDetected = true;
      }
    }
  ...
  public void cleanupExpiredFingerprints() {
    ...
    panFingerprints.values().forEach(list -> list.removeIf(fp -> fp.lastSeen().isBefore(cutoff)));

    // Remove empty entries
    panFingerprints.entrySet().removeIf(e -> e.getValue().isEmpty());

    log.info(
        "Fingerprint cleanup complete. Active PANs: {}, Active devices: {}",
        panFingerprints.size(),
        deviceToPans.size());
  }
