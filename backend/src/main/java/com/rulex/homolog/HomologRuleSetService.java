...
public class HomologRuleSetService {
    ...
    public RuleSetVersionResponse createDraft(String actorEmail, CreateRuleSetRequest req) {
        ...
        audit.success(actorEmail, AuditActionType.RULE_CREATED, "rule_sets", rs.getId().toString(),
    ...
    public RuleSetVersionResponse publish(String actorEmail, UUID ruleSetVersionId) {
        RuleSetVersionEntity v = ruleSetVersionRepository.findById(ruleSetVersionId)
        ...
        audit.success(actorEmail, AuditActionType.RULESET_PUBLISHED, "rule_set_versions", v.getId().toString(),
    ...
    public void activate(String actorEmail, UUID ruleSetVersionId) {
        ...
        ActiveRuleSetEntity active = activeRuleSetRepository.findById((short) 1).orElseGet(() -> {
        ...
        audit.success(actorEmail, AuditActionType.RULESET_ACTIVATED, "active_rule_set", "1",
                Map.of("ruleSetVersionId", v.getId().toString()),
                Map.of("message", "✅ RuleSet ativo atualizado"));
    }

    public SimulationResponse simulate(String actorEmail, UUID maybeRuleSetVersionId, TransactionRequest payload) {
        RuleSetVersionEntity v;
        if (maybeRuleSetVersionId != null) {
            v = ruleSetVersionRepository.findById(maybeRuleSetVersionId)
                    .orElseThrow(() -> new IllegalArgumentException("rulesetVersion não encontrado"));
        } else {
            ActiveRuleSetEntity active = activeRuleSetRepository.findById((short) 1)
                    .orElseThrow(() -> new IllegalStateException("Nenhum RuleSet ativo configurado"));
            v = ruleSetVersionRepository.findById(active.getRuleSetVersionId())
                    .orElseThrow(() -> new IllegalStateException("RuleSet ativo inválido"));
        }

        List<RuleSetVersionItemEntity> items = ruleSetVersionItemRepository.findByRuleSetVersionIdOrderBySortOrderAsc(v.getId());
        ...
        for (RuleSetVersionItemEntity item : items) {
            RuleVersionEntity rv = ruleVersionRepository.findById(item.getRuleVersionId())
                    .orElseThrow(() -> new IllegalStateException("ruleVersion referenciada não encontrada: " + item.getRuleVersionId()));
            if (!Boolean.TRUE.equals(rv.getEnabled())) {
                continue;
            }
            boolean match = evaluator.evaluate(sanitizer.sanitize(payload), rv.getLogic(), rv.getConditionsJson());
            if (!match) {
                continue;
            }
            ...
            TriggeredRuleDTO tr = TriggeredRuleDTO.builder()
                    .name(ruleName)
            ...
            } else if (rv.getDecision() == DecisionOutcome.SUSPEITA_DE_FRAUDE && finalDecision != DecisionOutcome.FRAUDE) {
        ...
        Map<String, Object> explain = Map.of(
        ...
        audit.success(actorEmail, AuditActionType.SIMULATION_RUN, "simulation_runs", sim.getId().toString(),
    ...
    private RuleSetVersionResponse toResponse(RuleSetEntity rs, RuleSetVersionEntity v, List<UUID> ruleVersionIds) {
