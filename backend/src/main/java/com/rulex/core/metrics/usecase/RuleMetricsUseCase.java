package com.rulex.core.metrics.usecase;
...
/** Use case para métricas de performance e eficácia das regras. */
public class RuleMetricsUseCase implements RuleMetricsInputPort {

  private final RuleMetricsRepositoryPort metricsRepository;

  public RuleMetricsUseCase(RuleMetricsRepositoryPort metricsRepository) {
  ...
  public void recordEvaluation(
    ...
    RuleMetrics metrics =
        metricsRepository
            .findByRuleIdAndMetricDate(ruleId, today)
            .orElseGet(
                () ->
                    RuleMetrics.builder()
                        .ruleId(ruleId)
  ...
  public RuleMetricsSummary getRuleMetrics(Long ruleId, LocalDate startDate, LocalDate endDate) {
    List<RuleMetrics> metrics =
        metricsRepository.findByRuleIdAndMetricDateBetweenOrderByMetricDateDesc(
    ...
    double triggerRate =
        totalEvaluations > 0 ? (double) totalTriggered / totalEvaluations * 100 : 0;
    double precision = totalTriggered > 0 ? (double) truePositives / totalTriggered * 100 : 0;
    double falsePositiveRate =
        totalTriggered > 0 ? (double) falsePositives / totalTriggered * 100 : 0;
  ...
  public MetricsDashboard getDashboard(LocalDate startDate, LocalDate endDate) {
    ...
    double overallFalsePositiveRate =
        totalTriggered > 0 ? (double) totalFalsePositives / totalTriggered * 100 : 0;

    List<RuleMetricsSummary> topRulesByTriggers =
        allMetrics.stream()
            .sorted(
                (a, b) -> Long.compare(b.getTotalTriggered(), a.getTotalTriggered()))
            .limit(10)
            .toList();

    List<RuleMetricsSummary> topRulesByAmountBlocked =
        allMetrics.stream()
            .sorted(
                (a, b) -> b.getTotalAmountBlocked().compareTo(a.getTotalAmountBlocked()))
            .limit(10)
            .toList();

    List<RuleMetricsSummary> problematicRules =
        allMetrics.stream()
            .filter(m -> m.getFalsePositiveRate() > 30.0)
            .limit(10)
            .toList();
    ...
    return MetricsDashboard.builder()
        ...
        .topRulesByTriggers(topRulesByTriggers)
        .topRulesByAmountBlocked(topRulesByAmountBlocked)
