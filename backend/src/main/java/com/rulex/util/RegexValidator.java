 ...
 *
 * <p>Implementa múltiplas camadas de proteção:
 * 1. Limite de tamanho do pattern
 * 2. Denylist de construções perigosas (catastrophic backtracking)
 * 3. Timeout na execução
 * 4. Limite de complexidade (grupos aninhados)
 */
@Slf4j
public final class RegexValidator {
     ...
     * Estes padrões podem causar tempo exponencial de execução.
     */
    ...
    public static ValidationResult validate(String pattern) {
        ...
        if (pattern.length() > MAX_PATTERN_LENGTH) {
            return ValidationResult.failure(
                String.format("Pattern muito longo (%d caracteres). Máximo: %d",
        ...
        if (dangerousMatch != null) {
            return ValidationResult.failure(
                String.format("Pattern contém construção perigosa que pode causar ReDoS: '%s'",
                    dangerousMatch));
        }
        ...
        if (complexity > 100) {
            return ValidationResult.failure(
                String.format("Pattern muito complexo (score: %d). Simplifique a expressão.", complexity));
        }
        ...
        if (captureGroups > MAX_CAPTURE_GROUPS) {
            return ValidationResult.failure(
                String.format("Muitos grupos de captura (%d). Máximo: %d",
                    captureGroups, MAX_CAPTURE_GROUPS));
        }
        ...
        if (nestedQuantifiers > MAX_NESTED_QUANTIFIERS) {
            return ValidationResult.failure(
                String.format("Muitos quantificadores aninhados (%d). Máximo: %d",
