import type { DiagramSource } from "../types";

type Props = {
  source: DiagramSource;
};

export default function PlantUmlRenderer({ source }: Props) {
  const code = source.kind === "inline" ? source.content : "";

  return (
    <div className="h-full w-full overflow-auto rounded-md border bg-muted/20 p-4">
      <div className="mb-3 text-sm text-muted-foreground">
        Renderer PlantUML pendente. Este viewer exibe o texto para futura
        integração (ex.: endpoint interno que retorna SVG/PNG).
      </div>
      <pre className="whitespace-pre-wrap rounded-md bg-background p-3 text-xs">
        {code || "(sem conteúdo)"}
      </pre>
    </div>
  );
}
