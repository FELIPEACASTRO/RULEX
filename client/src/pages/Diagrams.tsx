/**
 * Diagrams.tsx - Catálogo Completo de Diagramas e Fluxogramas
 *
 * Repositório visual de todos os tipos de diagramas usados no RULEX,
 * organizados por categoria: Processos, UML, Arquitetura, DDD, API,
 * Dados, Frontend, Infra e Segurança.
 */
...
const DIAGRAM_CATEGORIES: DiagramCategory[] = [
  {
    id: "process",
...
function DiagramCard({ diagram }: { diagram: DiagramType }) {
    ...
    <Card className="h-full">
      ...
      <CardContent className="space-y-3">
        <div className="text-sm">
          <span className="font-medium text-foreground">Uso no RULEX:</span>{" "}
          <span className="text-muted-foreground">{diagram.useCase}</span>
        </div>
        ...
        {diagram.example && (
          <Collapsible open={isOpen} onOpenChange={setIsOpen}>
            ...
            <CollapsibleContent>
              <pre className="mt-2 bg-muted rounded-lg p-3 text-xs overflow-x-auto font-mono whitespace-pre">
                {diagram.example}
              </pre>
            </CollapsibleContent>
          </Collapsible>
...
function CategorySection({ category }: { category: DiagramCategory }) {
      ...
      {category.subcategories ? (
        <Accordion type="multiple" className="space-y-2">
          {category.subcategories.map((sub) => (
            <AccordionItem key={sub.name} value={sub.name} className="border rounded-lg px-4">
              ...
              <AccordionContent>
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 pt-2">
                  {sub.diagrams.map((diagram) => (
                    <DiagramCard key={diagram.nameEn} diagram={diagram} />
                  ))}
                </div>
              </AccordionContent>
            </AccordionItem>
...
export default function Diagrams() {
  ...
  // Filtrar diagramas por busca
  const filterDiagrams = (diagrams: DiagramType[]) => {
    ...
    return diagrams.filter(
      (d) =>
        ...
        d.useCase.toLowerCase().includes(lower)
    );
  };

  const filteredCategories = DIAGRAM_CATEGORIES.map((cat) => ({
    ...cat,
    diagrams: cat.diagrams ? filterDiagrams(cat.diagrams) : undefined,
    subcategories: cat.subcategories?.map((sub) => ({
      ...sub,
      diagrams: filterDiagrams(sub.diagrams),
    })),
  })).filter((cat) => {
    if (cat.diagrams) return cat.diagrams.length > 0;
    if (cat.subcategories)
      return cat.subcategories.some((sub) => sub.diagrams.length > 0);
    return false;
  });

  // Contar total de diagramas
  const totalDiagrams = DIAGRAM_CATEGORIES.reduce((acc, cat) => {
    if (cat.diagrams) return acc + cat.diagrams.length;
    if (cat.subcategories)
      return acc + cat.subcategories.reduce((a, s) => a + s.diagrams.length, 0);
    return acc;
  }, 0);
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Catálogo completo de {totalDiagrams} tipos de diagramas para documentar o RULEX
          </p>
        </div>
      ...
      <div className="grid gap-4 grid-cols-2 md:grid-cols-4 lg:grid-cols-9">
        {DIAGRAM_CATEGORIES.map((cat) => {
          const Icon = cat.icon;
          const count = cat.diagrams
            ? cat.diagrams.length
            : cat.subcategories?.reduce((a, s) => a + s.diagrams.length, 0) || 0;
          return (
            <Card
              key={cat.id}
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        {DIAGRAM_CATEGORIES.map((cat) => (
          <TabsContent key={cat.id} value={cat.id} className="mt-6">
            <CategorySection
              category={
                filteredCategories.find((c) => c.id === cat.id) || cat
              }
            />
          </TabsContent>
        ))}
